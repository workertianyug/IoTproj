...
class ImageSender():
    ...
    def send_image_data(self, msg, image, dataDict):
        """Sends OpenCV image and msg to hub computer.
        ...
        if image.flags['C_CONTIGUOUS']:
            # if image is already contiguous in memory just send it
            self.zmq_socket.send_array_data(image, msg, dataDict, copy=False)
        else:
            # else make it contiguous before sending
            image = np.ascontiguousarray(image)
            self.zmq_socket.send_array_data(image, msg, dataDict, copy=False)
    ...
    def send_pos(self, x, y, text):

        self.zmq_socket.send_pos(x,y,text, copy=False)
        hub_reply = self.zmq_socket.recv()  # receive the reply message
        return hub_reply
    ...
    def send_text(self, msg):
        ...
        """
        self.zmq_socket.send_text(msg, copy=False)
        hub_reply = self.zmq_socket.recv()  # receive the reply message
        return hub_reply
...
class ImageHub():
    ...
    def recv_image_data(self, copy=False):
        """Receives OpenCV image and text msg.
        ...
        msg, image, md = self.zmq_socket.recv_array_data(copy=False)
        return msg, image, md
    ...
    def recv_pos(self, pos=False):

        x,y,text = self.zmq_socket.recv_pos(copy=False)
        return x,y,text

    def recv_text(self, copy=False):

        msg = self.zmq_socket.recv_text(copy=False)
        return msg 
...
class SerializingSocket(zmq.Socket):
    ...
    def send_array_data(self, A, dataDict, msg='NoName', flags=0, copy=True, track=False):
        """Sends a numpy array with metadata and text message.
    ...
    def recv_array_data(self, flags=0, copy=True, track=False):
        """Receives a numpy array with metadata and text message.
        ...
        msg = self.recv(flags=flags, copy=copy, track=track)
        A = np.frombuffer(msg, dtype=md['dtype'])
        return (md['msg'], A.reshape(md['shape']), md)
    ...
    def send_pos(self, x, y, text, msg='NoName', flags=0, copy=True, track=False):

        md = dict(x=x, y=y, text=text, )
        self.send_json(md, flags | zmq.SNDMORE)
        return self.send(b'nothing', flags, copy=copy, track=track)


    def recv_pos(self, flags=0, copy=True, track=False):

        md = self.recv_json(flags=flags)
        self.recv(flags=flags, copy=copy, track=track)
        return (md['x'], md['y'], md['text'])


    def send_text(self, text, msg='NoName', flags=0, copy=True, track=False):

        md = dict(text=text,)
        self.send_json(md, flags | zmq.SNDMORE)
        return self.send(b'nothing', flags, copy=copy, track=track)

    def recv_text(self, flags=0, copy=True, track=False):
